...
import numpy as np
from numpy import array, kron, trace, identity, sqrt, random, genfromtxt
import scipy
...
def doIterativeAlgorithm(numberOfQubits, bases, maxNumberOfIteration, listOfExperimentalData):
    """
    doIterativeAlgorithm():
        ...
        This recieve four variables (numberOfQubits, bases, maxNumberOfIteration, listAsExperimentalData),
...
def doPoissonDistirbutedSimulation(numberOfQubits, bases, maxNumberOfIteration, experimentalData, idealMatrix):
    """
    doPoissonDistributedSimulation(experimentalData, iterationTime)

    """

    estimatedDensityMatrix, timeDifference = doIterativeAlgorithm(numberOfQubits, bases, maxNumberOfIteration, random.poisson(experimentalData))
...
if __name__ == "__main__":
    ...
    # estimatedDensityMatrix, timeDifference = doIterativeAlgorithm(numberOfQubits, bases, maxNumberOfIteration, listOfExperimentalData)
    ...
    # fidelityList = doPoissonDistirbutedSimulation(numberOfQubits, bases, maxNumberOfIteration, listOfExperimentalData, matrix)
    ...
    with futures.ProcessPoolExecutor(max_workers=3) as executor:
        for _ in range(10):
            fidelity = executor.submit(fn=doPoissonDistirbutedSimulation, numberOfQubits=numberOfQubits, bases=bases, maxNumberOfIteration=maxNumberOfIteration, experimentalData=listOfExperimentalData, idealMatrix=matrix)
            fidelityList.append(fidelity)
        _ = futures.as_completed(fs=fidelityList)
    ...
    # for _ in range(10):
    #     fidelity = doPoissonDistirbutedSimulation(numberOfQubits, bases, maxNumberOfIteration, listOfExperimentalData, matrix)
    #     fidelityList.append(fidelity)
    ...
    # for i in range(10):
    #     process = Process(
    #         target=doPoissonDistirbutedSimulation,
    #         kwargs={
    #             'numberOfQubits':numberOfQubits,
    #             'bases':bases,
    #             'maxNumberOfIteration':maxNumberOfIteration,
    #             'experimentalData':listOfExperimentalData,
    #             'idealMatrix':matrix,
    ...
    # processes = [
    #     Process(target=doPoissonDistirbutedSimulation, args=)
